...
# noinspection DuplicatedCode,PyPep8Naming,PyUnresolvedReferences,PyAttributeOutsideInit,SpellCheckingInspection
class UIGraph:
    ...
    def UIGR_StopContinuousGraphRefresh(self):
        if self.isContinuousGraphRefreshEnabled is True:
    ...
    def UIGR_updateGraphsSimuTimer(self):
        if theConfig.CONFIG_INPUT_MODE_IS_REAL_MARKET is False:
            if self.totalNbIterations > theConfig.CONFIG_NB_POINTS_INIT_SIMU_GRAPH:
                ...
                if self.safeUIRefreshIsRequested is True:
                    self.safeUIRefreshIsRequested = False
        ...
        if theConfig.CONFIG_INPUT_MODE_IS_REAL_MARKET is True:
            ...
            if self.totalNbIterations > 1:  # Avoid computing min on a full of zeros array which throws an exception
                if self.totalNbGraphUpdates % 4 == 0:
                    ...
                    # Y range update only on change to avoid permanent axis rescaling which affects the user experience
                    # when zomming
                    if (self.minInPlot1 != minInPlot1) or (self.maxInPlot1 != maxInPlot1):
                        self.minInPlot1 = minInPlot1
                        self.maxInPlot1 = maxInPlot1
    ...
    def UIGR_updateInfoText(self, newInfoText, isError):
        ...
        if self.isContinuousGraphRefreshEnabled is False:
            ...
            if self.lblInfoInErrorStyle is True:
            ...
            if ("Welcome" in self.lblInfoStr) is True:
            ...
            elif ("here to unlock" in self.lblInfoStr) is True:
    ...
    def TimerRaisedBlinkWidgets(self):
        if self.isLblCurrentStateBlinking is True:
            if self.lblCurrentState.isVisible() is False:
                self.lblCurrentState.setVisible(True)
            else:
                self.lblCurrentState.setVisible(False)
